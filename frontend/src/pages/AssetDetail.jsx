      ...
      <header className="bg-white border border-slate-200 rounded-sm p-8">
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div className="max-w-2xl">
            <div className="text-[10px] font-mono text-slate-400 tracking-wider mb-2">{asset.id.toUpperCase()}</div>
            <h1 className="font-display text-4xl font-light tracking-tighter leading-tight">{asset.name}</h1>
            <p className="mt-3 text-base text-slate-600 leading-relaxed">{asset.description}</p>
        ...
        <div className="space-y-6">
          ...
          <SectionBlock title="Reusability">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm items-end">
              <div>
                <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold mb-2">Reuse Score</div>
                <div className="font-mono text-4xl font-medium tracking-tight text-slate-900">
          ...
          <SectionBlock title="Technical Details">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div>
                <div className="text-[10px] uppercase tracking-[0.2em] text-slate-500 font-bold mb-2">Tech Stack</div>
          ...
          <SectionBlock title="Constraints">
            <p className="text-sm text-slate-700 leading-relaxed">{asset.constraints}</p>
          </SectionBlock>
        </div>
        ...
        <aside className="space-y-6">
          <SectionBlock title="Related Assets">
            <div className="space-y-3">
              {related.length === 0 && <p className="text-xs text-slate-500">No related assets.</p>}
              {related.map((r) => (
                <Link
                  key={r.id}
                  to={`/assets/${r.id}`}
