    ...
    <div className="space-y-6" data-testid="explore-page">
      <div className="flex items-baseline justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">{filtered.length} of {projects.length} initiatives</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center border border-slate-200 dark:border-slate-800 rounded-sm bg-white dark:bg-slate-900 h-9">
            <SlidersHorizontal className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500 ml-3" />
            <select
              value={sort}
              onChange={(e) => setSort(e.target.value)}
              className="h-full bg-transparent px-2 text-sm text-slate-700 dark:text-slate-300 focus:outline-none"
          ...
          <div className="flex items-center border border-slate-200 dark:border-slate-800 rounded-sm bg-white dark:bg-slate-900 h-9">
            <button
              ...
              className={`h-full px-2 ${view === "grid" ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500"}`}
              data-testid="view-grid"
              title="Grid"
            ><LayoutGrid className="w-4 h-4" /></button>
            <button
              onClick={() => setView("list")}
              className={`h-full px-2 border-l border-slate-200 dark:border-slate-800 ${view === "list" ? "text-slate-900 dark:text-slate-100" : "text-slate-400 dark:text-slate-500"}`}
        ...
        <div data-testid="results-container">
          {filtered.length === 0 ? (
            <div className="border border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-sm p-16 text-center">
              <div className="font-display text-xl text-slate-900 dark:text-slate-100">No initiatives match</div>
              <p className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mt-1">Try clearing some filters or searching differently.</p>
              <button
                onClick={clearAll}
                className="mt-4 text-sm text-brand-700 dark:text-brand-400 hover:text-brand-800"
