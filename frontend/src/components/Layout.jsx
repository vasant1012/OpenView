...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900" data-testid="app-root">
      <header
        ...
        <div className="max-w-[1440px] mx-auto px-6 h-16 flex items-center gap-8">
          ...
          >
            ...
            {open && results && (results.projects.length + results.assets.length > 0) && (
              <div
                className="absolute top-full mt-1 left-0 right-0 bg-white border border-slate-200 rounded-sm overflow-hidden z-50"
                data-testid="global-search-results"
              >
                {results.projects.length > 0 && (
                  <div className="border-b border-slate-100">
                    <div className="px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] text-slate-400 bg-slate-50">
                      Initiatives
                    </div>
                    {results.projects.map((p) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-400 ml-3">{p.capability}</span>
                      </button>
                    ))}
                  </div>
                )}
                  ...
                  <div>
                    ...
                    {results.assets.map((a) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-400 ml-3">{a.type}</span>
                      </button>
                    ))}
                  </div>
                )}
      ...
      <footer className="border-t border-slate-200 mt-16 py-6 px-6 text-xs text-slate-500 flex items-center justify-between max-w-[1440px] mx-auto">
